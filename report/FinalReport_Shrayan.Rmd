...
\small{Estimating depth map from a single photograph is an intriguing problem with many interesting practical applications. Levin et al. \cite{levin2007image} proposed a method which requires a modified camera with a specialized coded aperture. However, this approach is not applicable in practice since it requires modifying the camera before capturing the image, which may not always be feasible. We aim to explore whether similar tasks can be done using standard cameras. We have formulated simple parametric models for depth blurring, which we hope will lead to more efficient estimates.}\\
 \small{ \textbf{Keywords:} AR Prior, Blur Kernel, Depth Map, Image Segmentation, Decorrelation Loss }
\end{abstract}
...
```{r,warning=FALSE,echo=FALSE,out.width='89%',out.height = '99%',fig.align='center',echo=FALSE,fig.cap = "Example of Depth Map from \\cite{levin2007image}. Original image (Left panel) and corresponding Depth map (right panel)"}
...
The assumption of locally constant blur is not always valid. In real images, level of blur tends to remain constant for different objects within the image, as it is the objects themselves that vary in distance from the camera, not the individual pixels. Figure \ref{fig:svar} provides an example illustrating this phenomenon, where different birds in the image exhibit varying levels of blur.
...
* Instead of working in the frequency domain, we can work in the spatial domain. For this we express (\ref{eq:3.5}) as $\text{Vec}(\boldsymbol{y}) = \boldsymbol{A_k} \text{Vec}(\boldsymbol{x}) + \text{Vec}(\boldsymbol{n})$, where $\text{Vec}(.)$ represents the corresponding vectorized versions of image matrices. We then formulate the maximum likelihood procedure as previously done in the frequency domain. However, this approach is computationally infeasible. To understand why, we assume IID prior on image gradients $\boldsymbol{x}$, with further elements of $\boldsymbol{n}$ also being independent. Then, $\text{Vec}(\boldsymbol{y})$ follows a multivariate normal distribution $\text{MVN}(\boldsymbol{0},\sigma^2A_kA^{T}_k + \eta^2I)$. To create a maximum likelihood procedure, we need to calculate the likelihood as a function of $\boldsymbol{k}$, i.e. as a function of $r$. However, this requires the calculation of an $MN \times MN$ matrix $\sigma^2A_kA^{T}_k + \eta^2I$ multiple times. Which makes it difficult to implement. 
