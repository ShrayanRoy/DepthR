...
```{r plot_3,warning=FALSE,echo=FALSE,out.width='95.5%',fig.align='center',echo=FALSE,fig.cap="Plot of ACF and PACF across and along the direction of horizontal gradients of eight test images in Figure \\ref{fig:plot_1}."}

knitr::include_graphics("C:/Users/User/Desktop/Github Projects/DepthR/report/images/3.3.png")
```
...
Using the above result, $\boldsymbol{|Y_\omega|^2} \sim \text{Exp}(\lambda_\omega = \frac{1}{\sigma^2|K_\omega|^2 g_{\omega} + \eta^2 h_{\omega}})$ $\boldsymbol{\forall \omega}$ independently. If $f_{\theta,\omega}(.)$ denotes the pdf of $\text{Exp}(\lambda_\omega = \frac{1}{\sigma^2|K_\omega|^2 g_{\omega} + \eta^2 h_{\omega}})$ for given parameters $\boldsymbol{\theta}$ (say,) of blur kernel model. Then, likelihood of $\boldsymbol{|Y_\omega|^2}$ is given by - 
