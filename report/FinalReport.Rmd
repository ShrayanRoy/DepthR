...
If we assume the elements of $\boldsymbol{x}$ are marginally distributed as (\ref{eq:3.2}) with correlation structure given by (\ref{eq:3.8}), then we can easily find an explicit expression for $g_{\omega}$. For the rest of the text we will work under this simple generalized lag-1 auto regressive model. $\rho_1$ and $\rho_2$ are hyperparameters and values of $\rho_1 = 0.3$ and $\rho_2 = 0.6$ are found to be appropriate for most of the images.  Note that in this case, $g_{\omega} = 1$ we have IID *Sparse MOG Prior*. 
