...
In a given camera setting (i.e., fixed $a_{diam}$ and $f$),^[For most cameras, $d_{focus} >> f$, hence the approximation.] $c_{diam} \propto \left|\frac{1}{d_{focus}} - \frac{1}{d}\right|$. Assuming a circular camera aperture, we expect the support of the PSF to be circular and intensity distribution of light to be spherical symmetrically distributed over the circular support. Keeping all these in mind we propose the following models for blur kernel.
...
```{r plot15,warning=FALSE,echo=FALSE,out.width='100%',out.height= '180%',fig.align='center',echo=FALSE,fig.cap = "Spatially Varying Blur Estimation: First Column for input blurry image, second column for estimated blur radius map and third column for deblurred image using estimated blur radius map in second column"}
...
In this project, we only considered the disc kernel for simulation and the circular cauchy kernel with $\kappa = 0.5$ for real image applications. However, we can generalize it for any spherically symmetric kernel. The choice of $\kappa$ is based on empirical observations, but we can also treat $\kappa$ as a parameter and incorporate into estimation procedure. Theoretically, for a single image, the choice of $\kappa$ should be the same for all segments (pixels), and one needs to consider this while estimating $\kappa$.
...
A R package implementing our estimation procedure is available on Github (`DepthR`: https://github.com/ShrayanRoy/DepthR) and the code for segmentation using SAM is also available.
