...
* Instead of working in the frequency domain, we can work in the spatial domain. For this we express (\ref{eq:3.5}) as $\text{Vec}(\boldsymbol{y}) = \boldsymbol{A_k} \text{Vec}(\boldsymbol{x}) + \text{Vec}(\boldsymbol{n})$, where $\text{Vec}(.)$ represents the corresponding vectorized versions of image matrices. We then formulate the maximum likelihood procedure as previously done in the frequency domain. However, this approach is computationally in-feasible. To understand why, we assume IID prior on image gradients $\boldsymbol{x}$, with further elements of $\boldsymbol{n}$ also being independent. Then, $\text{Vec}(\boldsymbol{y})$ follows a multivariate normal distribution $\text{MVN}(\boldsymbol{0},\sigma^2A_kA^{T}_k + \eta^2I)$. To form a maximum likelihood estimation procedure, we need to calculate the likelihood of $\boldsymbol{y}$ as a function of $\boldsymbol{\theta}$ (Which characterizes the blur kernel $\boldsymbol{k}$). For observed image gradient $y$ the log-likelihood is given by

\begin{equation}
\label{eq:9.1}
L(\boldsymbol{\theta}) \propto \frac{1}{\sqrt{\text{det}(\sigma^2A_kA^{T}_k + \eta^2I)}}\exp\left( -\frac{1}{2}y^T (\sigma^2A_kA^{T}_k + \eta^2I)^{-1} y\right)
\end{equation}

$\boldsymbol{\theta}$ is involved in $A_k$. But to find maximum likelihood estimator based on (\ref{eq:9.1}), we need to calculate inverse and determinant of $MN \times MN$ matrix $\sigma^2A_kA^{T}_k + \eta^2I$ multiple times. Which makes it difficult to implement for practical purpose.
