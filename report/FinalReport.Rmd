---
title: |
  | **Depth Estimation Using Segmentation in**
  | **Natural Images**
...
```{r,warning=FALSE,echo=FALSE,out.width='90%',out.height = '100%',fig.align='center',echo=FALSE,fig.cap = "Example of Depth Map from \\cite{levin2007image}. Original image (Left panel) and corresponding Depth map (right panel)"}
...
```{r svar,warning=FALSE,echo=FALSE,out.width='85%',out.height='75%',fig.align='center',echo=FALSE,fig.cap = "Example of Spatially Varying Blur: Each patch exhibits a distinct level of blur."}
...
To keep the notations simple, we will henceforth represent both (\ref{eq:3.3}) and (\ref{eq:3.4}) in the generic form
...
where $\boldsymbol{y}$ is the horizontal (or vertical) gradient of observed blurred image, and $\boldsymbol{x}$ and $\boldsymbol{n}$ are the same for latent image and noise. By virtue of the *Convolution Theorem*, we can rewrite (\ref{eq:3.5}) in the frequency domain as
...
where $\boldsymbol{Y,K,X}$ and $\boldsymbol{N}$ are the *Discrete Fourier Transform*'s of $\boldsymbol{y,k,x}$ and $\boldsymbol{n}$ respectively, and $\odot$ indicates the *element wise product* operator. For all $\boldsymbol{\omega} = (\omega_1,\omega_2)$ we have
...
If the elements of $\boldsymbol{x}$ are zero mean IID random variables with pdf (\ref{eq:3.1}), then by orthogonality of DFT, $\boldsymbol{X_{\omega}}$'s must be IID complex normal when $\alpha = 2$. Even for $\alpha \neq 2$, $\boldsymbol{X_{\omega}}$'s are  uncorrelated and asymptotically complex normal by the CLT of @peligrad2010. When elements of $\boldsymbol{x}$ are correlated, $\boldsymbol{X_{\omega}}$'s are still asymptotically independent and complex normal; however, depending on the correlation structure, the variance is no longer constant and depends on the specific frequency ${\omega}$. We will use either the IID prior, or the simple AR prior of @nandy2021dependent, for which $Var(\boldsymbol{X_{\omega}}) = g_{\omega}\sigma^2$ with the form of $g_{\omega}$ known explicitly.

For spatially varying blur, it is not immediately clear how we can express (\ref{eq:2.2}) in terms of image gradients. If we assume that the blur kernel $\boldsymbol{k_t}$ is shift invariant in a neighborhood ${\boldsymbol{\eta_t}}$ of size $p_1(\boldsymbol{t}) \times p_2(\boldsymbol{t})$ containing $\boldsymbol{t}$, then we get equation (\ref{eq:3.9}) as blur model. 
\begin{equation}
 \label{eq:3.9}
 \boldsymbol{y[t']} = (\boldsymbol{k_t} \ \otimes \ \boldsymbol{x})\boldsymbol{[t']} \ + \ \boldsymbol{n[t']} \ \ \ \forall \boldsymbol{t'} \in \boldsymbol{\eta_t}
\end{equation}

In practice, we expect this assumption to be approximately true if the patch size is small, because we expect objects in small local patches to have same depth and hence same level of blur (Figure \ref{fig:svar}). We can then apply the priors discussed above to these specific patches of the image.


# Parametric Models for Blur Kernel

From a single point source, light rays emit in different directions and fall on the lens of camera. The diffracted rays by the lens then form a circular shape on the camera sensor plane, which is called the *Circle of Confusion* or *Blur Circle* (Fig \ref{fig:plot_4}). The diameter of the blur circle ($c_{diam}$) and the depth of an object ($d$) in a given camera setting are related as follows [@barsky2003]:
...
\textbf{1. Disc Kernel:} This is the simplest blur model, which assumes uniform spread of light over a circular disc. We characterize the kernel using the parameter $r$, which corresponds to $c_{diam} / 2$ in (\ref{eq:4.1}).

$$k(x,y) = \frac{1}{\pi r^2} \times \text{I}_{\{x^2 + y^2 \ \leq \ r^2\}}$$

\textbf{2. Circular Gaussian Kernel:} Another standard choice of kernel the widely used Gaussian/Normal distribution. Here, we will consider a truncated version of it over a circular region. We characterize the kernel using the radius of circle $r$ and scale parameter $h$.
...
Note that each value of $c_{diam}$ corresponds to two possible values of the depth $d$, either closer to or further away from the camera relative to the focal plane. Thus, from the diameter of the blur circle, which is the parameter we estimate, it is impossible to distinguish between these two possibilities.
...
Our goal is to find the $\boldsymbol{\theta}$ maximizing $L(\boldsymbol{\theta})$, or equivalently $\log L(\boldsymbol{\theta}) = \log L_h(\boldsymbol{\theta}) + \log L_v(\boldsymbol{\theta})$. We do this by numerically optimizing the objective (log-likelhood) function. For the spatially varying case, we can apply this procedure to local patches $\boldsymbol{\eta_t}$ for all pixel locations $\boldsymbol{t}$ in the image domain.

# Choice of Tuning Parameters in ML Estimation


The log-likelihood, defined in (\ref{eq:5.2}), is a complicated function of the blur kernel parameter $\boldsymbol{\theta}$. This parameter is involved in the expression for $\lambda_{\omega}$ through $|K_{\omega}|^2$, which itself is a complicated function of $\boldsymbol{\theta}$. Therefore, before we start using any optimization method to find the maximizer of the log-likelihood, we should investigate the behavior of $L(\boldsymbol{\theta})$ as a function of $\boldsymbol{\theta}$. For this, we conducted simulated experiments as shown in Figure \ref{fig:plot6}. For now, we will focus on the disc kernel. We considered a sequence of values for $r$ ranging from 1 to 4 with a step size of $\Delta{r} = 0.05$, and for $\sigma$ ranging from 0.01 to 0.20 with a step size of $\Delta{\sigma} = 0.01$, while keeping $\eta = 0.005$ constant.


```{r plot6,warning=FALSE,echo=FALSE,out.width='87%',out.height= '60%',fig.align='center',echo=FALSE,fig.cap = "(a) 101 $\\times$ 101 Sharp Image, (b) Blurred Image Using disc kernel with $r = 3$, (c) Disc Kernel with $r = 3$, (d) Levelplot of log likelihood as a function of $\\sigma$ and $r$"}
...
```{r plot7,warning=FALSE,echo=FALSE,out.width='87%',out.height= '60%',fig.align='center',echo=FALSE,fig.cap = "(a) 101 $\\times$ 101 Sharp Image, (b) Blurred Image Using disc kernel with $r = 2$, (c) Disc Kernel with $r = 2$, (d) Levelplot of log likelihood as a function of $\\sigma$ and $r$"}
...
# Considerations in Spatially Varying Blur Estimation


We have developed a procedure for estimating blur kernel parameters for images with uniform blur. However, real-world images often exhibit spatially varying blur, where the level of blur varies from pixel to pixel. To address this, one reasonable assumption is that the level of blur remains locally constant within a small patch around each pixel. By applying our estimation procedure to each local patch, we can obtain the blur level for each pixel, as described in Equation (\ref{eq:3.9}). This results in an estimated blur map for the image.

In Figure \ref{fig:plot110}, we have tried to estimate spatially varying blur for image of moon. But, instead of estimating the blur level for each pixel, we divided the image into 56 equal parts and for each part, we estimated the blur kernel using the maximum likelihood procedure assuming circular Cauchy kernel with $\kappa = 0.5$ to estimate the blur radius. The estimated blur kernels are overlaid on the image, part by part, for visualization. As expected, the blur radius map is spherically symmetric with minimum value at the center and it increases as we move away from center.

In Figure \ref{fig:plot11}, we have conducted a simple experiment using spatially varying blur with a disc kernel on a $255\times255$ image. We have used local patches of size $51\times 51$ for estimation of blur radius for each pixel. Both the original and estimated blur maps are shown. It is evident that the estimated blur map closely resembles the original blur map except for the transition regions, with mean squared error 0.492.

```{r plot11,warning=FALSE,echo=FALSE,out.width='85%',out.height= '55%',fig.align='center',echo=FALSE,fig.cap = "Spatially Varying Blur Estimation Under Locally Constant Assumption: (a) Spatially Varying Blurred Image, (b) Latent Blurmap, (c) Estimated Blurmap"}
...
```{r plot110,warning=FALSE,echo=FALSE,out.width='90%',out.height= '160%',fig.align='center',echo=FALSE,fig.cap = "Spatially Varying Blur for Moon: (a) Image of Moon, (b) Blur radius map using circular cauchy kernel with $\\kappa = 0.5$ (black for small radius and white for large), (c) Estimated blur kernels are overlaid on the image part by part"}

knitr::include_graphics("images/moon_map.png")
```
...
```{r plot12,warning=FALSE,echo=FALSE,out.width='80%',out.height= '45%',fig.align='center',echo=FALSE,fig.cap = "Automatic and Guided Segmentation Using Segment Anything Model"}
...
For post-segmentation estimation of the blur map, we need to apply the maximum likelihood procedure to each image segment identified by SAM. Unfortunately, this is not straightforward, because the maximum likelihood procedure as outlined above requires rectangular images, whereas the image segments obtained are of irregular shape. The obvious solution is to fill / pad the segments with zeros to make it a rectangular array; however, this approach typically leads to bias towards selecting smaller values of the radius $r$. The issue arises because the discrete Fourier transform (DFT) combines frequency content from across all spatial locations, implicitly assuming stationarity of the gradients, which no longer holds.

Some approaches we tried to address this issue are the following.

\begin{itemize}

\item {Instead of utilizing the irregular segments identified by SAM, we tried using the corresponding bounding boxes as inputs. However, bounding boxes may encompass objects with vastly different depths, leading to significantly varied blur levels within the same box. In such scenarios, we've observed that the sub-area of the box with the least level of blur tends to dominate the Fourier spectrum.} 

\item {Another strategy is to decompose irregularly shaped segments into smaller, rectangular regions and apply the estimation procedure to these sub regions. However, this approach possibly discards a large proportion of the available data depending on how irregular the segment is.}

\item { Instead of working in the frequency domain, we can work in the spatial domain. As convolution is a linear operator, we can express (\ref{eq:3.5}) as $\text{vec}(\boldsymbol{y}) = \boldsymbol{A_k} \text{vec}(\boldsymbol{x}) + \text{vec}(\boldsymbol{n})$, where $\text{vec}(.)$ represents the corresponding vectorized versions of image segments, and $\boldsymbol{A_k}$ is a matrix determined by the blur kernel $\boldsymbol{k}$. We can then formulate the maximum likelihood procedure as previously done in the frequency domain. However, this approach is computationally infeasible. To understand why, we first consider the simpler IID prior on image gradients $\boldsymbol{x}$, assuming further that elements of $\boldsymbol{n}$ are also independent. Then, $\text{vec}(\boldsymbol{y})$ follows a multivariate normal distribution $\text{MVN}(\boldsymbol{0}, \sigma^2 \boldsymbol{A_k} \boldsymbol{A_k}^{T} + \eta^2 \boldsymbol{I})$. To form a maximum likelihood estimation procedure, we need to calculate the likelihood (given $\boldsymbol{y}$) as a function of $\boldsymbol{\theta}$ (which characterizes the blur kernel $\boldsymbol{k}$). For observed image gradient $\boldsymbol{y}$ the likelihood is given by

\begin{equation}
\label{eq:9.1}
L(\boldsymbol{\theta}) \propto \frac{1}{\sqrt{\text{det}(\sigma^2 \boldsymbol{A_k} \boldsymbol{A_k}^{T} + \eta^2 \boldsymbol{I})}}\exp\left( -\frac{1}{2}y^T (\sigma^2 \boldsymbol{A_k} \boldsymbol{A_k}^{T} + \eta^2 \boldsymbol{I} )^{-1} y\right)
\end{equation}

The parameter $\boldsymbol{\theta}$ determines $\boldsymbol{A_k}$. To evaluate the likelihood function using (\ref{eq:9.1}), we need to calculate the inverse and determinant of the matrix $\sigma^2 \boldsymbol{A_k} \boldsymbol{A_k}^{T} + \eta^2 \boldsymbol{I}$. Although this matrix is sparse, it is still potentially large, and evaluating the likelihood for multiple values of $\theta$ for each segment is impractical.}

\end{itemize}

In the next section, we describe an ad hoc alternative loss function that is simpler to evaluate.


# Decorrelation Loss Function

<!-- It is evident from the preceding discussion that relying on methods involving padding or filling zeros to transform irregularly shaped segments into rectangular arrays is unreliable. Instead, our method should solely depend on the pixel values within the identified image segments. An obvious choice is to collect those pixel values in matrix and use that for analysis. However this is not theoretically valid. Because the spatial dependency of the pixels in image carries information about the blur kernel parameters. Arranging them without maintaining their spatial order would destroy their dependency structure. This leads to the idea of *Decorrelation Loss Function*. -->

For an observed blurred image $\boldsymbol{y}$, if the true blur kernel is $\boldsymbol{k_{r_0}}$, it is evident from Equation (\ref{eq:3.5}) that deconvolving $\boldsymbol{y}$ using $\boldsymbol{k_{r_0}}$ would yield a quantity $\boldsymbol{x_{r_0}}$ closely resembling the latent sharp image gradient $\boldsymbol{x}$. Consequently, the autocorrelation function of the horizontal and vertical gradients of $\boldsymbol{x_{r_0}}$ would exhibit smaller values compared to the blurred image gradient. To empirically validate this, we simulated defocus blur on a $255 \times 255$ image using a disc kernel of radius $r_{\text{true}} = 3$. We have used a deconvolution procedure similar to Wiener filtering to obtain $\boldsymbol{x_r}$. For comparison, we have plotted ACF for blurred image gradients, deconvoluted gradients using $r = 2,3,4$ and also of sharp images in Figure \ref{fig:plot13}.

```{r plot13,warning=FALSE,echo=FALSE,out.width='100%',out.height= '100%',fig.align='center',echo=FALSE,fig.cap = "Motivation for Decorrelation loss: Top row indicates ACF for sharp image gradients, second row for blurred image gradients and last three rows for deconvoluted gradients. The correct value of the radius is 3."}
...
Using Equation (\ref{eq:10.1}) we have obtained the deconvoluted versions of both the horizontal and vertical gradients in the frequency domain, and then performed an Inverse Discrete Fourier Transform (IDFT) to obtain their spatial domain counterparts, denoted by $\boldsymbol{x_{h,r}}$ and $\boldsymbol{x_{v,r}}$ respectively. Then, we calculate the autocorrelation function (ACF) along and across the gradient directions for both the horizontal and vertical deconvoluted gradients. Typically, considering only the ACFs along the direction is sufficient. However, we have observed that including both along and across direction ACFs results in more stable outcomes. We select the value of $r$ that minimizes the decorrelation loss.

For image segments, we slightly modify the procedure. We perform deconvolution of bounding boxes corresponding to the image segments and instead of calculating the ACF for all pixels within that bounding box, we only consider the pixels that belong to the image segment corresponding to that bounding box. 

The choice of the parameters $\sigma$ and $\eta$ are also important. We have conducted an experiment similar to maximum likelihood case to obtain optimal choice of parameters and the results are similar. Choice of $\rho_1$ and $\rho_2$ are also important. The default choices are $\rho_1 = 0.5$ and $\rho_2 = 0.5$.

# Simulated Experiments

A simulated experiment is illustrated in Figure \ref{fig:plot14}, which allows us to quantitatively test the performance of the proposed method. We start with a sharp original RGB image and apply SAM to identify the image segments. SAM considers different bounding boxes and produces three possible masks for each such bounding box along with their confidence scores. We discard small segments based on the size of the bounding box and the confidence score of the corresponding mask. Afterward, we simulate defocus blur based on these masks using a disc kernel, employing the model in (\ref{eq:3.9}) and considering the segments as neighbors of the pixel they belong to. The latent blur map and generated blurred image are shown in Figure \ref{fig:plot14} (a) and (c). We apply the above-discussed method to the gray scale version of the blurred image with $\sigma = 0.10$, $\eta = 0.005$, and $\rho = (0.5,0.5)$ as the choice of tuning parameters. The estimated blur map is shown in Figure \ref{fig:plot14} (d). We have used a grid-search procedure with step size $\Delta{r} = 0.1$ to search for optimal $r$. 

<!-- The mean squared error of the estimated blur map with respect to the latent map is 0.421. -->

```{r plot14,warning=FALSE,echo=FALSE,out.width='70%',out.height= '60%',fig.align='center',echo=FALSE,fig.cap = "Simulated Experiment: (a) Simulated input image with spatially varying blur, (b) Deblurred image based on estimate in (d), (c) Latent blur map, (d) Estimated blur map"}
...
The input images in Figure \ref{fig:plot16} are from @levin2007image. In each case, the blur maps depict the nearly correct blur level estimation, and the deblurred images also show improvement. Figure \ref{fig:plot17} (a),(b) presents close-ups of faces from \ref{fig:plot16} (a),(c) respectively before and after deblurring, demonstrating the improvement achieved. The deblurred images are not smooth, but we can achieve smoothness using other deconvolution algorithms, For example putting $L_p$ penalty on deblurred image gradients [@nandy2021dependent]. But those are much slower compared to Richardson-Lucy. 

Finally, in Figure \ref{fig:plot17} we show some comparisons from examples given in @zhu2013. A notable improvement is the accurate identification of boundaries between objects in the blur maps, which is a consequence of using SAM to perform pre-segmentation. Our method clearly depicts the differences in blur levels, such as between the cattle in (e), the individuals and buildings in (d), whereas these details are absent in [@zhu2013]. The ranking is not entirely correct, but it provides a better representation of 3D geometric information compared to [@zhu2013].

```{r plot15,warning=FALSE,echo=FALSE,out.width='100%',out.height= '200%',fig.align='center',echo=FALSE,fig.cap = "Spatially Varying Blur Estimation: First Column for input blurry image, second column for estimated blur radius map and third column for deblurred image using estimated blur radius map in second column"}

knitr::include_graphics("images/12.1.png")
```

```{r plot16,warning=FALSE,echo=FALSE,out.width='100%',out.height= '180%',fig.align='center',echo=FALSE,fig.cap = "Spatially Varying Blur Estimation: First Column for input blurry image, second column for estimated blur radius map and third column for deblurred image using estimated blur radius map in second column"}

knitr::include_graphics("images/12.2.png")
```

```{r plot17,warning=FALSE,echo=FALSE,out.width='100%',out.height= '160%',fig.align='center',echo=FALSE,fig.cap = "Closeups and Comparison with our method with \\cite{zhu2013}: (a), (b) shows improvment after deblurring and (c), (d), (e), (f) are comparison of blurmaps"}
...
Our initial goal was to estimate depth using the blur level as a surrogate. However, this is an ill-posed problem because, for a given value of $r$ (i.e., $c_{diam}/2$), there can be two different values of $d$, as seen from Equation \ref{eq:4.1}. However, the blur maps can be considered as depth maps if the object closest to the camera in the image is the sharpest in the scene. The images in Figure \ref{fig:plot16} are examples of this, where the corresponding blur map can be viewed as depth map.

The ranking in blur maps is not entirely correct. One common issue is identifying the blur level of background objects. Additionally, segments with very little texture often result in underestimation of the blur radius. This issue is common to most passive algorithms but can be mitigated using a post-segmentation approach with neighboring pixel information. In [@zhu2013], the initial blur map estimate also had ambiguities, but after MRF smoothing, they have obtained a reasonable blur map. We need to think whether a post-estimation modification can remove the ambiguities in our blur maps.

In this project, we only considered the disc kernel for simulation and the circular cauchy kernel with $\kappa = 0.5$ for real image applications. However, we can generalize it for any spherically symmetric kernel. The choice of $\kappa$ is based on empirical observations, but we can also treat $\kappa$ as a parameter and incorporate into estimation procedure. For a single image, the choice of $\kappa$ should be the same for all segments (pixels), and one needs to consider this while estimating $\kappa$.

The choice of tuning parameters $\sigma$, $\eta$, and $\rho$ is crucial. We based our selection of these parameters on a simulation study. However, when dealing with real images, we may need to examine them more closely. It's true that selecting the right tuning parameters can improve our blur map estimation process. But it is hard to isolate the effects of each tuning parameter individually because of their confounding effect. 

The blur maps estimated by our method clearly depict the differences in blur levels and boundaries between objects in image, unlike those in [@zhu2013; @levin2007image]. This improvement is mainly due to segmenting the image into meaningful objects and using that for estimation. It reduces computation time significantly. Because, instead of estimating for each pixel separately, we estimate only for segments, which is significantly fewer than the number of pixels. Currently our R implementation takes around 7 minutes to perform the estimation step for a $1000 \times 1000$ image on CPU run time and additional 3 minutes for segmentation using SAM on GPU run time in Google Colab Platform. In contrast, methods based on estimating the blur level for each pixel separately take around 1 hour and 30 minutes only for the initial blur map estimation on the same platform and run time. This demonstrates a significant improvement in computation time.

# Supplementary Materials: `DepthR`

A R package implementing our estimation procedure is available on Github named `DepthR` (https://github.com/ShrayanRoy/DepthR). It has implementation of both maximum likelihood procedure and decorrelation loss. The code for segmentation using SAM is also available.

# Acknowledgement

First and foremost, I would like to express my heartfelt gratitude to my supervisor Dr. Deepayan Sarkar, for his unwavering support and deep involvement throughout my project. His exemplary mentorship and expertise were instrumental in the successful completion of my work.

I am indebted to the faculty members at ISI Delhi and Kolkata for their continuous support throughout my M.Stat Programme, nurturing my growth at every step. Their guidance, both in and out of the classroom, provided the foundation upon which this thesis stands.

Finally, I express my gratitude to my family and friends for their unwavering support and encouragement throughout this endeavor. Their belief in me fueled my determination to persevere.

<!-- Specially, I would like to mention the name of Adrija Saha, for her constant encouragement throughout my whole academic journey.-->
