...
[TODO: Mention Noise Case, Change Images, eta = 0.005 etc.]
...
The Segment Anything Model (SAM) developed by Meta AI in 2023 revolutionizes image segmentation by offering versatile modes of use, such as - automatic segmentation, guided segmentation (**Figure**). SAM employs a unique network architecture similar to language models such as *ChatGPT* and *Google Bard*. It utilizes an *image encoder* for embedding computation, a *prompt encoder* for prompt embedding, and a *lightweight decoder* for mask prediction. SAM's data engine collected over 1.1 billion high-quality masks through three stages: Assisted-Manual, Semi-Automatic, and Fully-Automatic, resulting in the *SA-1B dataset*, a significant contribution to the image processing field. SAM's promptable architecture enables zero-shot learning, resolving ambiguity in prompts, and tackling various downstream tasks effectively. With its potential for adaptation to diverse image-processing tasks, several application specific modifications of SAM are available, such as **Ref**. 

For our use, we have used SAM for automatic segmentation. In this mode SAM considers first uniform grid of points over the image. Then based on that it selects some bounding boxes, which represents  
...
For an observed blurred image $\boldsymbol{y}$, if the true blur kernel is $\boldsymbol{k_{r_0}}$, it is evident from Equation (\ref{eq:3.5}) that deconvolving $\boldsymbol{y}$ using $\boldsymbol{k_{r_0}}$ would yield a quantity $\boldsymbol{x_{r_0}}$ closely resembling the original sharp image $\boldsymbol{x}$. Consequently, the autocorrelation function (ACF) of the horizontal and vertical gradients of $\boldsymbol{x_{r_0}}$ would exhibit small values. To empirically validate this, we simulated defocus blur on a $255 \times 255$ image using a disc kernel of radius $r_{\text{true}} = 3$ as shown in **Figure**. We have used a deconvolution procedure similar to wiener filtering to obtain $\boldsymbol{x_r}$.

For $r = 2$, we can see that ACF values are smaller compared to the blurred image, and for $r = 4$, at some lags, the ACF takes a large value. But for the true value of $r$, we observe a substantial decrease in ACF values as expected. It is to be noted that the ACF in the across direction has not decreased much. 

Based on the observation, we consider sum of squared acfs as loss function. It captures the amount of correlation present in image gradients after deconvoluting using $\boldsymbol{k_r}$. For true value of $r$, we expect this to be small. We have used the following theorem for deconvolution step.
...
A simulated experiment is illustrated in **Figure**, which allows us to quantitatively test the performance of the proposed method. We start with a sharp original RGB image and apply SAM to identify the image segments. SAM considers different bounding boxes and produces three possible masks for each such bounding box along with their confidence scores. We discard small segments based on the size of the bounding box and the confidence score of the corresponding mask. After that, we simulate defocus blur based on these masks using a disc kernel using the model in (\ref{eq:3.9}). The latent blur map and generated blurred image are shown in **Figure**. We apply the above-discussed method to the grayscale version of the blurred image with $\sigma = 0.05$, $\eta = 0.010$, and $\rho = (0.5,0.5)$ as the choice of tuning parameters. The estimated blur map is shown in **Figure**. The mean squared error of the estimated blur map with respect to the latent map is **Value**.

Notice that except for some regions, the estimated blur map closely resembles the latent blur map. The discrepancy is mainly for small segments. Using the estimated blur map and blur kernels generated by the disc function, we performed spatially varying deconvolution using the procedure described in @levin2007image. The deblurred image is shown in **Figure**. We can see that the spatially varying blur is successfully removed from the image. The PSNR of the estimate $\boldsymbol{\hat{x}}$ of the sharp image $\boldsymbol{x}$ is **Value**, which is quite high.
